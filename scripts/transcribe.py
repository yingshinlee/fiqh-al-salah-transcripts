...
流程：
  ...
  3. 自動偵測 Whisper 的「重複辨識錯誤」段落（某字不斷重複），切片重轉修正
...
def is_repetition_error(text: str) -> bool:
    core = "".join(c for c in text if c not in PUNCT)
    if len(core) < 12:
        return False
    # 同一個字連續重複 8 次以上 → 幾乎一定是辨識陷入重複迴圈
    if longest_run(core) >= 8:
        return True
    # 整段去重後字種極少（重複短語）→ 也視為重複辨識錯誤
...
def main():
    ...
    with tempfile.TemporaryDirectory() as workdir:
        ...
        # 偵測 + 修正重複辨識錯誤段
        bad = [i for i, s in enumerate(segs) if is_repetition_error(s["text"])]
        if bad:
            print(f"🩹 偵測到 {len(bad)} 段重複辨識錯誤，切片重轉修正…", flush=True)
        for i in bad:
            s = segs[i]
            ...
            try:
                ...
                r = mlx_whisper.transcribe(
                    ...
                    condition_on_previous_text=False,  # 關掉前文制約，避免再次陷入重複迴圈
                    initial_prompt=args.prompt, verbose=False,
                )
                fixed = collapse_repeats(r["text"])
                # 若重轉後仍有重複，至少把原本的重複壓掉
                segs[i]["text"] = fixed if not is_repetition_error(fixed) else collapse_repeats(s["text"])
        ...
        print(f"\n✅ 完成（{len(segs)} 段，修正 {len(bad)} 處重複辨識錯誤）")
