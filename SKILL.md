...
description: >-
  ...
  Whisper 在結尾或中段常見的「某個字不斷重複」辨識錯誤。當使用者想把 YouTube 影片、講座、
...
腳本會自動：下載音訊 → 辨識 → **偵測並切片重轉修正重複辨識錯誤** → 輸出 txt 與 srt。
...
校對完成後，依需要可以再產出這幾種版本（同一份逐字稿衍生）：

1. **全形標點版**：把純文字稿 `.txt` 的半形 `, ? !` 轉成全形 `，？！`（中文閱讀慣例）。注意**不要動到**經文章節號（如 `4:103` 的冒號）與阿拉伯文行內的標點。
2. **字幕最佳化 `.srt`**：給影片作者上字幕用。把長句依標點切短（每句約 ≤20 全形字）、時間按字數比例分配；**移除括號內的羅馬拼音註解**（畫面字幕只留乾淨中文，例如「阿布·胡萊賴（Abū Hurayrah）」→「阿布·胡萊賴」）；過長的阿拉伯文誦讀改成簡短標記。為避免某些播放器顯示方框，可把學術音標改成安全拼法（Ṣalawāt→Salawat）。
3. **可讀版**（見下）。

## 「可讀版」的做法與尺度

有時使用者想要一份「給人讀」的版本。**關鍵：這不是把逐字稿改寫成文章，而是讓它讀起來仍是一篇乾淨的口述逐字稿。** 請嚴格遵守以下尺度，否則容易過度加工：

**要做的（只做這些）：**
- 保留講者的**口述語氣、第一人稱、原本的講述順序**。
- 刪掉贅字與口頭禪：「就是」「就是說」「那個」「呢」「啊」「嘛」「對不對」「OK 好」等。
- 刪掉**重複**（如「最核心的知識嘛，最核心的知識嘛」「一部分一部分一部分」）與**結巴／自我重啟**（如「我是不知道啦，我是不知道說」）。
- 把原本被切成「一行一句」的破碎片段，**併成順暢的段落**；段落只在自然語氣轉折處分段。
- 全形標點。

**不要做的：**
- ❌ 不要加小標、不要列點、不要分章節。
- ❌ 不要改寫句子結構、不要把口語改成書面語的腔調。
- ❌ 不要替講者「修正內容或事實」。事實存疑處用 `〔註：…〕` 標註，不直接改（例如講者口誤的稱謂、年份）。

**幾個慣例：**
- 長段的阿拉伯文誦讀，用「（講者誦讀…）」一句帶過即可，完整原文留在 `.txt` / `.srt`。
- 已和使用者**確認過**的人名／術語修正可套用；**尚未確認**的地方保留原貌。
- 產出存成 `<名稱>_可讀版.md`，並在開頭用一句話註明「保留口述語氣、僅刪贅字、未改寫」。
